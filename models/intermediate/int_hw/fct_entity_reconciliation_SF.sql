...
WITH
    ...
    */
    ...
    ledger_stg AS (
        ...
        FROM {{ ref('revenue_subledger_preliminary') }}
        {{ date_between }}
    ),
    ...
    billing_stg AS (
        ...
        FROM {{ ref('base_headway__billing_appointment_details') }}
        {{ date_between }}
    ),
            ...
            */
            ...
            CASE
                ...
                WHEN ledger_entity_state_from_name IS NOT NULL
                    ...
                    AND LOWER(ledger_entity_name) != LOWER(billing_entity_name)
            ...
            END AS reconciliation_classification_by_precedence,

            /*
                Broad reconciliation family:
                - record presence issue
                - entity data quality issue
                - reconciled
                - approved business exception
                - entity mismatch

                Specific reconciliation subtype:
                - exact mutually exclusive outcome within the family
            */

            CASE
                -- one system has the appointment record, the other does not
                WHEN has_billing_record AND NOT has_ledger_record
                    THEN 'record_presence_issue'

                WHEN has_ledger_record AND NOT has_billing_record
                    THEN 'record_presence_issue'

                -- both records exist, but entity fields are incomplete on one or both sides
                WHEN has_ledger_record
                    AND has_billing_record
                    AND (
                        (billing_entity_name IS NULL OR billing_entity_tin IS NULL)
                        OR (ledger_entity_name IS NULL OR ledger_entity_tin IS NULL)
                    )
                    THEN 'entity_data_quality_issue'

                -- exact reconciliation
                WHEN has_ledger_record
                    AND has_billing_record
                    AND billing_entity_name IS NOT NULL
                    AND billing_entity_tin IS NOT NULL
                    AND ledger_entity_name IS NOT NULL
                    AND ledger_entity_tin IS NOT NULL
                    AND LOWER(ledger_entity_name) = LOWER(billing_entity_name)
                    AND ledger_entity_tin = billing_entity_tin
                    THEN 'reconciled'
                ...
                WHEN has_ledger_record
                    ...
                    AND ledger_entity_name IS NOT NULL
                    AND ledger_entity_tin IS NOT NULL
                    AND is_known_optum_pair_mismatch
                    AND LOWER(ledger_entity_name) != LOWER(billing_entity_name)
                    THEN 'approved_business_exception'

                -- all remaining complete two-sided mismatches
                ELSE 'entity_mismatch'
            END AS reconciliation_classification,

            CASE
                -- record presence issues
                WHEN has_billing_record AND NOT has_ledger_record
                    THEN 'missing_in_ledger'
                ...
                -- entity data quality issues
                WHEN has_ledger_record
                    AND has_billing_record
                    AND (billing_entity_name IS NULL OR billing_entity_tin IS NULL)
                    AND (ledger_entity_name IS NULL OR ledger_entity_tin IS NULL)
                    THEN 'missing_entity_fields_in_both'
                ...
                WHEN has_ledger_record
                    ...
                    AND ledger_entity_name IS NOT NULL
                    AND ledger_entity_tin IS NOT NULL
                    AND NOT is_known_optum_pair_mismatch
                    AND LOWER(ledger_entity_name) = LOWER(billing_entity_name)
                    AND ledger_entity_tin != billing_entity_tin
                    THEN 'tin_mismatch_same_name'
                ...
                WHEN has_ledger_record
                    ...
                    AND ledger_entity_tin IS NOT NULL
                    AND NOT is_known_optum_pair_mismatch
                    AND ledger_entity_state_from_name IS NOT NULL
                    AND billing_entity_state_from_name IS NOT NULL
                    AND ledger_entity_state_from_name != billing_entity_state_from_name
                    AND LOWER(ledger_entity_name) != LOWER(billing_entity_name)
                    THEN 'cross_state_entity_mismatch'
                ...
                ELSE 'unclassified_entity_mismatch'
            END AS reconciliation_sub_classification
    ...
    summary AS (
        SELECT
            ...
            reconciliation_sub_classification,
